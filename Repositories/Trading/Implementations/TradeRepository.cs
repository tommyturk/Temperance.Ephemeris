...
namespace Temperance.Ephemeris.Data.Repositories.Trading.Implementations
{
    public class TradeRepository : ITradeRepository
    {
        ...
        public async Task CheckTradeExitsAsync()
        {
            ...
            var openTrades = await connection.QueryAsync<Trade>(query);

            foreach (var trade in openTrades)
            {
                decimal latestPrice = 0.0m;// await _historicalPricesRepository.GetLatestPriceAsync(trade.Symbol, trade.SignalTimestamp);
